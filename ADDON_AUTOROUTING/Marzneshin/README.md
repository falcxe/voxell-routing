...
curl -fLo /var/lib/marzneshin/subscription.py \
  https://raw.githubusercontent.com/falcxe/voxell-routing/main/ADDON_AUTOROUTING/Marzneshin/subscription.py
```
...
```yaml
    environment:
      VOXELL_ROUTING_SOURCE: "default"   # default | jsonsub | whitelist | custom
      # VOXELL_ROUTING_CUSTOM: "happ://..."  # только при source=custom
```
...
| `custom` | Ваша произвольная ссылка из `VOXELL_ROUTING_CUSTOM` |
...
Готово! Happ-клиенты при следующей синхронизации подписки получат Voxell Routing в заголовке `Routing:`.

## Как это работает

- `subscription.py` монтируется поверх оригинального файла Marzneshin.
- При отдаче подписки он подтягивает содержимое `.DEEPLINK`-файла из [Voxell Routing](https://github.com/falcxe/voxell-routing) с **кешем 10 минут** — никаких блокирующих запросов на каждый ответ.
